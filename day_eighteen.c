    ...
    while (fgets(buffer, sizeof(buffer), fp) != NULL)
    {
        ...
        while (*p && *p != '\n')
        {
            ...
            if (isdigit(c))
            {
                ...
                PUSH(num_stack, last_op == '+' ? (last_num + this) : (last_num * this));
            }
            else if (c == '+' || c == '*')
            {
            ...
            {
                // Add this dummy here so that we have something at the top of the
                // stack to operate with when we hit the first number.
                PUSH(num_stack, 0);
                PUSH(op_stack, '+');
            }
            else if (c == ')')

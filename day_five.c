...
void day_five_b_solution(FILE* fp) {
    ...
    // Loop through, skipping the first and last seats. If we find a hole with
    // both seats filled next to it then it will be ours.
    for (int i = 1; i < (128 * 8) - 1; ++i) {
        if (bitvector_check(bv, i - 1) && bitvector_check(bv, i + 1) &&

...
static int read_input(FILE *fp, seating_chart buffer_a, seating_chart buffer_b, int *rows, int *columns)
{
    int bytes_read = 0;
...
static int get_los_occupied(seating_chart seats, int row_idx, int col_idx, int rows, int cols)
{
    int occupied = 0;
    ...
    for (int i = 0; i < 8; ++i)
    {
        ...
        do
        {
            ...
            if (adjacent_column < 0 || adjacent_column >= cols || adjacent_row < 0 || adjacent_row >= rows)
            {
                // out of bounds
                stop = true;
                continue;
...
static int get_adjacent_occupied(seating_chart seats, int row_idx, int col_idx, int rows, int cols)
{
    int occupied = 0;
    int adjacent_row, adjacent_column;
...
static int game_step(seating_chart current, seating_chart previous, int rows, int columns, bool puzzle_two)
{
...
static bool compare_boards(seating_chart board_a, seating_chart board_b, int rows, int cols)
{
    for (int i = 0; i < rows; ++i)
    {
...
void day_eleven_solution(FILE *fp)
{
    run_game(fp, false /* puzzle two */);
}

void day_eleven_b_solution(FILE *fp)
{
    run_game(fp, true /* puzzle two */);
}
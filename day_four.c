...
static bool is_valid_year_field(const char *s, int len, const char *prefix, int min_year, int max_year)
{
...
static bool is_valid_ecl_field(const char *s, int len)
{
    ...
    const char *color_part = s + 4;
    if (strcmp("amb", color_part) == 0 ||
        strcmp("blu", color_part) == 0 ||
        strcmp("brn", color_part) == 0 ||
        strcmp("gry", color_part) == 0 ||
        strcmp("grn", color_part) == 0 ||
        strcmp("hzl", color_part) == 0 ||
...
void day_four_b_solution(FILE *fp)
{
    ...
    while (fgets(buffer, sizeof(buffer), fp) != NULL)
    {
        ...
        while (found)
        {
            ...
            else if (!state->has_valid_iyr && is_valid_iyr_field(found, found_len))
            {
                state->has_valid_iyr = true;
            }
            else if (!state->has_valid_eyr && is_valid_eyr_field(found, found_len))
            {
                state->has_valid_eyr = true;
            }
            else if (!state->has_valid_hgt && is_valid_hgt_field(found, found_len))
            {
                state->has_valid_hgt = true;
            }
            else if (!state->has_valid_hcl && is_valid_hcl_field(found, found_len))
            {
                state->has_valid_hcl = true;
            }
            else if (!state->has_valid_ecl && is_valid_ecl_field(found, found_len))
            {
                state->has_valid_ecl = true;
            }
            else if (!state->has_valid_pid && is_valid_pid_field(found, found_len))
            {
                state->has_valid_pid = true;
            }

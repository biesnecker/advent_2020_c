...
static void
update_neighborhood_3d(neighbor_grid_3d neighbors, int w, int h, int d) {
...
static void
update_neighborhood_4d(neighbor_grid_4d neighbors, int w, int h, int d, int t) {
    for (int x = w - 1; x < w + 2; ++x) {
        for (int y = h - 1; y < h + 2; ++y) {
            for (int z = d - 1; z < d + 2; ++z) {
...
static void update_neighbors_3d(active_grid_3d active,
                                neighbor_grid_3d neighbors) {
...
static void update_neighbors_4d(active_grid_4d active,
                                neighbor_grid_4d neighbors) {
...
static void step_3d(active_grid_3d current,
                    active_grid_3d previous,
                    neighbor_grid_3d neighbors) {
...
static void step_4d(active_grid_4d current,
                    active_grid_4d previous,
                    neighbor_grid_4d neighbors) {
    int active_neighbors = 0;
...
void day_seventeen_solution(FILE* fp) {
    ...
    while (fgets(buffer, sizeof(buffer), fp) != NULL) {
        for (int x = 0; x < STARTING_WIDTH; ++x) {
            ...
            if (buffer[x] == '#') {
                active_b[x + N_ROUNDS + 1][y + N_ROUNDS + 1][N_ROUNDS + 1] =
                    true;
            }
        }
        ++y;

...
static void read_input(FILE *fp, rule *rules, int *rules_len,
                       ticket *yourticket, int *fields_len,
                       ticket *nearby_tickets, int *tickets_len)
{
    (void)(yourticket);
    (void)(nearby_tickets);
    char buffer[100] = {0};
    ...
    while (fgets(buffer, sizeof(buffer), fp) != NULL)
    {
        if (mode == 0)
        {
            ...
            rules[rule_idx].is_departure = strstr(buffer, "departure") == buffer;
            rules[rule_idx].fields = 0xFFFFF;
            char *range_start = strchr(buffer, ':') + 2;
            sscanf(range_start, "%d-%d or %d-%d",
...
static bool has_invalid_fields(const ticket *t, int fields_len, const rule *rules, int rules_len, int *sum)
{
    int total = 0;
    int v = 0;
    bool failed = true;
...
void day_sixteen_solution(FILE *fp)
{
    ...
    read_input(fp, rules, &rules_len, &yourticket, &fields_len, tickets, &tickets_len);

    int invalid_sum = 0;

    for (int i = 0; i < tickets_len; ++i)
    {
        has_invalid_fields(&tickets[i], fields_len, rules, rules_len, &invalid_sum);
    }
    printf("Solution: %d\n", invalid_sum);
}

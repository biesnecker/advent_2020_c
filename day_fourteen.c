...
static int create_masks(const char *mstr, uint64_t *or_mask, uint64_t *and_mask)
{
    uint64_t _or_mask = 0;
...
static void update_or_insert(memloc *memory, uint64_t location, uint64_t value)
{
...
void day_fourteen_b_solution(FILE *fp)
{
    ...
    while (fgets(buffer, sizeof(buffer), fp) != NULL)
    {
        ...
        else
        {
            ...
            // Zero out the bits that have an X. The last of the masks is always the all-ones
            // one so use its inverse.
